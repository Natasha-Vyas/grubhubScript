...
async function getResponse(merchantUuid, token, sequelize, id, sequelize1) {
    ...
    try {
        ...
        {
            method: 'get',
            url: `https://api-order-processing-gtm.grubhub.com/merchant/accounting/v1/${merchantUuid}/transactions?timeZone=America/Chicago&startDate=2021-03-01&endDate=2021-04-12`,
            headers: {
...
async function storeData(orders, id, sequelize1) {
    try {
        ...
        if (orders.order_id) {
            let customer_id = await sequelize1.query('Select customer_id from  `customer` where customer_id=?', { replacements: [orders.order_id], type: QueryTypes.SELECT });
            if (customer_id.length)
                await sequelize1.query('update   `customer` set name="test",phone="0000" where customer_id=?', { replacements: [customer_id[0].customer_id], type: QueryTypes.INSERT });
            else
                await sequelize1.query('insert into customer (customer_id,name,phone) values(?,?,?)', { replacements: [orders.order_id, "test","0000"], type: QueryTypes.INSERT });
        }
    ...
    if (orders.order_id) {
        ...
        if (order_id.length)
            ...
            await sequelize1.query('insert into `order` (order_id,sub_total,tax,total,branch_platform_id,customer_id,status,note,order_recieved_time,order_recieved_date,order_confirmed_date,order_ready_date,order_out_for_delivery_date) values(?,?,?,?,?,?,?,?,?,?,?,?,?)',
                {
                    replacements: [
                        orders.order_id,
                        orders.sub_total_charges,
...
async function run2(merchantUuid, token, transId, item, sequelize, id, sequelize1) {
    ...
    try {
        ...
        for (let i = 0; i < 1; i++) {
            ...
            // console.log(transformer,"llllllllllllllll")
